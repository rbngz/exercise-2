...
def astar_search(
    ...
    @param heuristic  A heuristic callable which computes the estimated steps
                      from a search node to reach the goal.
    @param make_open_entry An optional parameter to change the behavior of the
    ...
    while open:
        ...
        if h < besth:
            besth = h
            logging.info("Found new best h: %d after %d expansions" %
                         (besth, counter))
        ...
        if pop_g != state_cost[pop_state]:
            ...
            expansions += 1
            logging.info("Expansions: {}".format(expansions))
            ...
            successor_states = task.get_successor_states(pop_state)
            ...
            for op, new_state in successor_states:
                # For every neighbor state:
                # Step 7.2: Create a neighbor node with make_child_node()
                neighbor_node = searchspace.make_child_node(
                    pop_node, op, new_state)
                ...
                #           for the root node)
                h_cost = heuristic(neighbor_node)
                ...
                # Step 8.2: Store the g cost of the neighbor node to the state_cost dictionary
                    state_cost[new_state] = neighbor_node.g
                ...
                #           See also above, how the root node was stored in the heap.
                    heapq.heappush(open, make_open_entry(
                        neighbor_node, h_cost, node_tiebreaker))
